...
class User extends Authenticatable implements FilamentUser, HasDefaultTenant, HasTenants, MustVerifyEmail
{
    ...
    public function canAccessPanel(Panel $panel): bool
    {
        if ($panel->getId() === 'admin') {
            return ! $this->staff_access_disabled_at && $this->hasAnyRole(['super_admin', 'admin']);
        }

        // The non-admin panel is the authenticated customer/team workspace.
        return true;
    }
    ...
    protected function casts(): array
    {
        return [
            ...
            'staff_access_disabled_at' => 'datetime',
    ...
    protected static function booted(): void
    {
        static::updating(function (self $user): void {
            if ($user->isDirty(['password', 'email']) && ! $user->isDirty('security_version')) {
                $user->security_version = (int) $user->getRawOriginal('security_version') + 1;
            }
        });
    }
